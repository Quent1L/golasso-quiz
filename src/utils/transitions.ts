...
export function usePageTransitions() {
  ...
  watch(
    ...
    (newRouteName, oldRouteName) => {
      ...
      isNavigatingBack.value = newLevel < oldLevel || 
        (newLevel === oldLevel && navigationHistory.value.indexOf(newRouteString) < navigationHistory.value.indexOf(oldRouteString))
      
      // Choisir la transition appropriée
      transitionName.value = getTransitionForRoute(newRouteString, oldRouteString, isNavigatingBack.value)
    },
    { immediate: true }
  )
